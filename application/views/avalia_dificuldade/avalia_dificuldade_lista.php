...
if (validation_errors()){
    //echo "Comparação=".(trim(validation_errors()) == trim("O campo nome da avalia_dificuldade é obrigatório."));
    $erro_qtdobrigatorio = "O campo nome do tipo de dificuldade é obrigatório.";
            ...
            <div class="col-lg-12">
                <table class="table table-striped table-hover">
                    <tr>
                        ...
                        ?>
                            <i onclick="mudaAction('<?php echo base_url('avalia_dificuldade/avalia_dificuldades/gerar_excel'); ?>');" style="cursor: pointer;">
                                <img width="30px" height="30px"  src="<?php  echo base_url('/assets/images/excel.png');?>"/>
                            </i>
                            <i onclick="mudaAction('<?php echo base_url('avalia_dificuldade/avalia_dificuldades/gerar_pdf'); ?>');" style="cursor: pointer;">
                                <img width="30px" height="30px"  src="<?php  echo base_url('/assets/images/pdf.png');?>"/>
                            </i>
                        <?php }?>
                        </td>
                    </tr>
                    <!-- Inicio lista de avalia_dificuldades encontradas na consulta -->
                    <?php
                    if($total_registros>0){
                        foreach ($registros as $result) {
                            ...
                            <tr>
                                <td>
                                    <a type="button"
                                    href="<?php echo base_url('avalia_dificuldade/avalia_dificuldades/editar/'.$result->ci_avalia_dificuldade); ?>"
                                    style="width: 80px;height: 33px"
                                    class="btn btn-custom waves-effect waves-light btn-micro active">Editar</a>

                                </td>
                                <td ><?php echo $result->ci_avalia_dificuldade; ?></td>
                                <td><?php echo $result->nm_avalia_dificuldade; ?></td>
                                <td align="right">

                                    <button type="button"
                                            class="btn btn-danger waves-effect waves-light btn-micro active"
                                            onclick="javascript:excluir('<?php echo $result->ci_avalia_dificuldade;?>');">
                                        Excluir
                                    </button>
