...
if (validation_errors()){
    //echo "Comparação=".(trim(validation_errors()) == trim("O campo nome da grupo é obrigatório."));
            ...
            <div class="col-lg-12">
                <table class="table table-striped table-hover">
                    <tr>
                        ...
                        ?>
                            <!-- <i onclick="mudaAction('<'?php echo base_url('usuario/grupos/gerar_excel'); ?>');" style="cursor: pointer;">
                                <img width="30px" height="30px"  src="<'?php  echo base_url('/assets/images/excel.png');?>"/>
                            </i>
                            <i onclick="mudaAction('<'?php echo base_url('usuario/grupos/gerar_pdf'); ?>');" style="cursor: pointer;">
                                <img width="30px" height="30px"  src="<'?php  echo base_url('/assets/images/pdf.png');?>"/>
                            </i> -->
                        <?php }?>
                        </td>
                    </tr>
                    <!-- Inicio lista de grupos encontradas na consulta -->
                    <?php
                    if($total_registros>0){
                        foreach ($registros as $result) {
                            ...
                            <tr>
                                <td>
                                    <a type="button"
                                    href="<?php echo base_url('usuario/grupos/editar/'.$result->ci_grupousuario); ?>"
                                    style="width: 80px;height: 33px"
                                    class="btn btn-custom waves-effect waves-light btn-micro active">Editar</a>

                                </td>
                                <td ><?php echo $result->ci_grupousuario; ?></td>
                                <td><?php echo $result->nm_grupo; ?></td>
                                <td align="right">

                                    <button type="button"
                                            class="btn btn-danger waves-effect waves-light btn-micro active"
                                            onclick="javascript:excluir('<?php echo $result->ci_grupousuario;?>');">
                                        Excluir
                                    </button>
