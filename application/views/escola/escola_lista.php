...
if (validation_errors()){
    //echo "Comparação=".(trim(validation_errors()) == trim("O campo nome do escola é obrigatório."));
    $erro_qtdobrigatorio = "O campo nome da escola é obrigatório.";
            ...
            <div class="col-lg-12">
                <table class="table table-striped table-hover">
                    <tr>
                        ...
                        if($total_registros<0){
                        ?>
                            <i onclick="mudaAction('<?php echo base_url('escola/escolas/gerar_excel'); ?>');" style="cursor: pointer;">
                                <img width="30px" height="30px"  src="<?php  echo base_url('/assets/images/excel.png');?>"/>
                            </i>
                            <i onclick="mudaAction('<?php echo base_url('escola/escolas/gerar_pdf'); ?>');" style="cursor: pointer;">
                                <img width="30px" height="30px"  src="<?php  echo base_url('/assets/images/pdf.png');?>"/>
                            </i>
                        <?php }?>
                        </td>
                    </tr>
                    <!-- Inicio lista de escolas encontradas na consulta -->
                    <?php
                    if($total_registros>0){
                        foreach ($registros as $result) {
                            ...
                            <tr>
                                <td>
                                    <a type="button"
                                    href="<?php echo base_url('escola/escolas/editar/'.$result->ci_escola.'/'.$result->cd_estado); ?>"
                                    ...
                                    <td ><?php echo strtoupper ($result->nm_escola); ?></td>
                                    ...
                                    <button type="button"
                                            ...
                                            onclick="javascript:excluir('<?php echo $result->ci_escola;?>');">
                                        Excluir
                                    </button>
