...
if (validation_errors()){
    //echo "Comparação=".(trim(validation_errors()) == trim("O campo nome da transacao é obrigatório."));
    $erro_qtdobrigatorio = "O campo nome do transação é obrigatório.";
            ...
            <div class="col-lg-12">
                <table class="table table-striped table-hover">
                    <tr>
                        ...
                        ?>
                            <!-- <i onclick="mudaAction('<'?php echo base_url('usuario/transacoes/gerar_excel'); ?>');" style="cursor: pointer;">
                                <img width="30px" height="30px"  src="<'?php  echo base_url('/assets/images/excel.png');?>"/>
                            </i>
                            <i onclick="mudaAction('<'?php echo base_url('usuario/transacoes/gerar_pdf'); ?>');" style="cursor: pointer;">
                                <img width="30px" height="30px"  src="<'?php  echo base_url('/assets/images/pdf.png');?>"/>
                            </i> -->
                        <?php }?>
                        </td>
                    </tr>
                    <!-- Inicio lista de transacoes encontradas na consulta -->
                    <?php
                    if($total_registros>0){
                        foreach ($registros as $result) {
                            ...
                            <tr>
                                <td>
                                    <a type="button"
                                    href="<?php echo base_url('usuario/transacoes/editar/'.$result->ci_transacao); ?>"
                                    style="width: 80px;height: 33px"
                                    class="btn btn-custom waves-effect waves-light btn-micro active">Editar</a>

                                </td>
                                <td ><?php echo $result->ci_transacao; ?></td>
                                <td><?php echo $result->nm_transacao; ?></td>
                                <td align="right">

                                    <button type="button"
                                            class="btn btn-danger waves-effect waves-light btn-micro active"
                                            onclick="javascript:excluir('<?php echo $result->ci_transacao;?>');">
                                        Excluir
                                    </button>
