...
if (validation_errors()){
    //echo "Comparação=".(trim(validation_errors()) == trim("O campo nome da deficiencia é obrigatório."));
    $erro_qtdobrigatorio = "O campo nome da deficiencia é obrigatório.";
            ...
            <div class="col-lg-12">
                <table class="table table-striped table-hover">
                    <tr>
                        ...
                        ?>
                            <!-- <i onclick="mudaAction('<'?php echo base_url('deficiencia/deficiencia/gerar_excel'); ?>');" style="cursor: pointer;">
                                <img width="30px" height="30px"  src="<'?php  echo base_url('/assets/images/excel.png');?>"/>
                            </i>
                            <i onclick="mudaAction('<'?php echo base_url('deficiencia/deficiencia/gerar_pdf'); ?>');" style="cursor: pointer;">
                                <img width="30px" height="30px"  src="<'?php  echo base_url('/assets/images/pdf.png');?>"/>
                            </i> -->
                        <?php }?>
                        </td>
                    </tr>
                    <!-- Inicio lista de deficiencia encontradas na consulta -->
                    <?php
                    if($total_registros>0){
                        foreach ($registros as $result) {
                            ...
                            <tr>
                                <td>
                                    <a type="button"
                                    href="<?php echo base_url('deficiencia/deficiencia/editar/'.$result->ci_tipodeficiencia); ?>"
                                    style="width: 80px;height: 33px"
                                    class="btn btn-custom waves-effect waves-light btn-micro active">Editar</a>

                                </td>
                                <td ><?php echo $result->ci_tipodeficiencia; ?></td>
                                <td><?php echo $result->ds_tipodeficiencia; ?></td>
                                <td align="right">

                                    <button type="button"
                                            class="btn btn-danger waves-effect waves-light btn-micro active"
                                            onclick="javascript:excluir('<?php echo $result->ci_tipodeficiencia;?>');">
                                        Excluir
                                    </button>
